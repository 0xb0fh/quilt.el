...
(defun quilt-unapplied-list ()
  (split-string (quilt-cmd-to-string "unapplied") "\n" t))
...
(defun quilt-diff-z ()
  ...
  (interactive)
  (quilt-save)
  (quilt-cmd "diff -z" quilt--diff-buf))

(defun quilt-diff-patch ()
  "Diff against a specified patch"
  (interactive)
  (let* ((arg (quilt-complete-list "Patch name: " (quilt-patch-list))))
       (quilt-save)
       (quilt-cmd (concat "diff -P " arg) quilt--diff-buf)))
...
(defun quilt-merge ()
  "Fold named patch into the topmost applied"
  (interactive)
  (let* ((arg (quilt-complete-list "Fold with patch: " (quilt-unapplied-list))))
       (quilt-save)
       (quilt-cmd (concat "fold < " (quilt-dir) "/patches/" arg))
       (if (y-or-n-p (format "Want to delete %s from disk? " arg))
	   (quilt-cmd (concat "delete -r " arg)))))
...
(define-key quilt-mode-map "\C-cqz" 'quilt-diff-z)
(define-key quilt-mode-map "\C-cqP" 'quilt-diff-patch)
...
(define-key quilt-mode-map "\C-cqM" 'quilt-merge)
